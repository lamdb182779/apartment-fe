...
export default function OwnerList({ name }: { name: string }) {
    const [page, setPage] = useState(1)
    const { data, isLoading } = useSWR(`owners?current=${page}&name=${name}`, fetcher)
    const router = useRouter()
    const handleCreate = () => {
        router.push("/receptionist/residents/add")
    }
    return (
        <>
            {isLoading === false &&
                                ...
                                <TableBody>
                                    {data?.results.map((item: any) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell>{item.apartments.map((item: any) => item.number).join(", ")}</TableCell>
